...
public class BrandService extends BaseService {
	 ...
	 * @throws Exception
	 */
	public boolean getAttnedExist(String brandName) throws Exception {

		String sql = "SELECT count(0) as cnt FROM tbdaily.tb_tran_month_prop t where t.tran_month = ? and t.prop_name = '品牌' and t.prop_value = ?";

		Map<String, Object> baseMap = sqlUtil.search(sql, DateUtils.getOffsetMonth(-1, "yyyy-MM"), brandName);
	 ...
	 * @throws Exception
	 */
	public String attnedBrand(String uid, String brandName) throws Exception {

		boolean exist = getAttnedExist(brandName);
		if (exist) {// 存在则添加到关注列表
	 ...
	 * @throws Exception
	 */
	public List<Map<String, Object>> getAttnBrand(String uid, String q) throws Exception {

		String sql = "select t1.brand_name, t2.addid from ("
				+" SELECT distinct t.prop_value as brand_name FROM tbdaily.tb_tran_month_prop t where t.tran_month = ? and t.prop_name = '品牌' and t.prop_value like '%"+ q+ "%') t1"
				+" left join tbweb.tb_attn_brand t2 on t1.brand_name = t2.brand_name and t2.uid = ? order by t1.brand_name limit 50";
		
		return sqlUtil.searchList(sql, DateUtils.getOffsetMonth(-1, "yyyy-MM"), uid);
	 ...
	 * @throws Exception
	 */
	public boolean enabledDel(String uid, String brandNames) throws Exception {

		String sql = "select count(0) as cnt from tbweb.tb_attn_brand where brand_name in (" + StringUtils.strIn(brandNames)
	 ...
	 * @throws Exception
	 */
	public void delAttn(String uid, String brandNames) throws Exception {

		String sql = "delete FROM tbweb.tb_attn_brand where brand_name in (" + StringUtils.strIn(brandNames) + ") and uid = ?";
	...
	public PageEntity<BrandList> getBrandAnalysisList(String uid, PageParam pageParam) throws Exception{
		...
		List<BrandList> list = sqlUtil.searchList(BrandList.class, pageParam.buildSql(sql), DateUtils.getCurMonth(), DateUtils.getOffsetMonth(-1, "yyyy-MM"), uid);
